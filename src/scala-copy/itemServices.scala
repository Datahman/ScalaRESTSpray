...
class ItemServices(implicit val ec: ExecutionContext) {
  ...
  def validateItemOfferExpiry(timearray: Array[(String, Long)], itemtobeValidated: Future[Option[Item]]) = {
    timeNow = System.currentTimeMillis() / 1000
    val pattern = """(\d+)([s]|[m]|h|d)""".r
    var itemName = Await.result(itemtobeValidated.map(x => x.get.itemName), 5.seconds)
        ...
        case someT => someT match {
          case pattern(offerInt, offerPeriod) =>
            ...
            if (offerInt.toInt > 0 && offerPeriod == "h") {
              val timeaggregate = offerInt.toInt * 60* 60 + time._2
              if (timeaggregate - timeNow > 0) {
                timeleftInhours = abs(timeaggregate - timeNow) / 60 * 60
                state = "on"
              state}
              else {state = "off"}
            }
  ...
  def getAllItems(): Future[List[Item]] = Future {
    for (i <- items) {
      validateItemOfferExpiry(getItemTimeProperties(i.itemID), getItem(i.itemID))
      if (state == "off") {
